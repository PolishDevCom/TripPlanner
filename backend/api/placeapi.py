...
class PlacesApiRequest:
    ...
    def make_venue_list(self):
        ...
        for i in range(self.limit):
            item = {
                "name": self.response["groups"][0]["items"][i][
                    "venue"
                ]["name"],
                "categories": self.response["groups"][0]["items"][
                    i
                ]["venue"]["categories"][0]["name"],
                "address": self.response["groups"][0]["items"][i][
                    "venue"
                ]["location"]["formattedAddress"],
                "lattitude": self.response["groups"][0]["items"][
                    i
                ]["venue"]["location"]["lat"],
                "longitude": self.response["groups"][0]["items"][
                    i
                ]["venue"]["location"]["lng"],
                "distance": self.response["groups"][0]["items"][i][
                    "venue"
                ]["location"]["distance"],
                "id": self.response["groups"][0]["items"][i][
                    "venue"
                ]["id"],
            }
...
class VenueApiRequest:
    ...
    def get_venue_details(self):
        categories_length = len(
            self.response["venue"]["categories"]
        )
        venue_categories = []
        ...
        details = {
            ...
            "address": self.response["venue"]["location"][
                "formattedAddress"
            ],
